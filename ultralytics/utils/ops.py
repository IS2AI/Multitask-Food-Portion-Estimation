...
def non_max_suppression(
    ...
    nm = prediction.shape[1] - nc - 4 - 1  # number of masks
    mi = 4 + 1 + nc  # mask start index
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]) and not rotated:
            ...
            v[range(len(lb)), lb[:, 0].long() + 4 + 1] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        box, weight, cls, mask = x.split((4, 1, nc, nm), 1)

        if multi_label:
            i, j = torch.where(cls > conf_thres)
            x = torch.cat((box[i], x[i, 4 + 1 + j, None], j[:, None].float(), mask[i], weight[i]), 1)
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            lb = labels[xi]
            v = torch.zeros((len(lb), nc + nm + 1), device=x.device)
            v[:, :4] = lb[:, 1:5]  # box
        ...
        box,other, cls, mask = x.split((4,1, nc, nm), 1)

        if multi_label:
            i, j = torch.where(cls > conf_thres)
            x = torch.cat((box[i], x[i, 4+1 + j, None], j[:, None].float(), mask[i],other[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), mask,other), 1)[conf.view(-1) > conf_thres]
...
def xywh2xyxy(x):
    ...
    """
    #assert x.shape[-1] == 4, f"input shape last dimension expected 4 but input shape is {x.shape[-1]}"
    y = empty_like(x)  # faster than clone/copy
    xy = x[..., :2]  # centers
