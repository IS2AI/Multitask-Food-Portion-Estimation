    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                assert (lb[:, 1:5] <= 1).all(), \
                        f'non-normalized or out of bounds coordinates {lb[:, 1:5][lb[:, 1:5] > 1]}'
                assert (lb[:, 1:5] >= 0).all(), f'negative label values {lb[lb[:, 1:5] < 0]}'
...
def check_dual_dataset(dataset, autodownload=True):
    ...
    # Checks
    for k in 'train_rgb', 'val_rgb', 'train_depth', 'val_depth':
        if k not in data:
            raise SyntaxError(
                emojis(f"{dataset} '{k}:' key missing ❌.\n'train_rgb','train_depth', 'val_rgb' and 'val_depth' are required in all data YAMLs."))
    if 'names' not in data and 'nc' not in data:
        raise SyntaxError(emojis(f"{dataset} key missing ❌.\n either 'names' or 'nc' are required in all data YAMLs."))
    if 'names' in data and 'nc' in data and len(data['names']) != data['nc']:
        raise SyntaxError(emojis(f"{dataset} 'names' length {len(data['names'])} and 'nc: {data['nc']}' must match."))
    ...
    if not path.is_absolute():
        path = (DATASETS_DIR / path).resolve()
    data['path'] = path  # download scripts
    for k in 'train_rgb', 'val_rgb', 'test_rgb', 'train_depth', 'val_depth', 'test_depth':
        if data.get(k):  # prepend path
            if isinstance(data[k], str):
                x = (path / data[k]).resolve()
                if not x.exists() and data[k].startswith('../'):
    ...
    # Parse yaml
    train_rgb, val_rgb, test_rgb, train_depth, val_depth, test_depth, s = (
        data.get(x) for x in ('train_rgb', 'val_rgb', 'test_rgb', 'train_depth', 'val_depth', 'test_depth', 'download')
    )
    if val_rgb:
        val_rgb = [Path(x).resolve() for x in (val_rgb if isinstance(val_rgb, list) else [val_rgb])]  # val RGB path
        if not all(x.exists() for x in val_rgb):
            name = clean_url(dataset)  # dataset name with URL auth stripped
            m = f"\nDataset '{name}' RGB images not found ⚠️, missing paths %s" % [str(x) for x in val_rgb if not x.exists()]
            if s and autodownload:
                LOGGER.warning(m)
            else:
            ...
            dt = f'({round(time.time() - t, 1)}s)'
            s = f"success ✅ {dt}, saved to {colorstr('bold', DATASETS_DIR)}" if r in (0, None) else f'failure {dt} ❌'
            LOGGER.info(f'Dataset RGB download {s}\n')
            
    # Validate Depth paths
    if val_depth:
        val_depth = [Path(x).resolve() for x in (val_depth if isinstance(val_depth, list) else [val_depth])]  # val Depth path
        if not all(x.exists() for x in val_depth):
            name = clean_url(dataset)  # dataset name with URL auth stripped
            m = f"\nDataset '{name}' Depth images not found ⚠️, missing paths %s" % [str(x) for x in val_depth if not x.exists()]
            if s and autodownload:
                LOGGER.warning(m)
            else:
                m += f"\nNote dataset download directory is '{DATASETS_DIR}'. You can update this in '{SETTINGS_YAML}'"
            ...
            dt = f'({round(time.time() - t, 1)}s)'
            s = f"success ✅ {dt}, saved to {colorstr('bold', DATASETS_DIR)}" if r in (0, None) else f'failure {dt} ❌'
            LOGGER.info(f'Dataset Depth download {s}\n')        
