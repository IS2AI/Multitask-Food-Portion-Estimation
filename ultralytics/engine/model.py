    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        if Path(weights).suffix == ".pt":
            ...
            if hasattr(self.model, "model"):
                # Compute the index of the detection head (assumed to be the last module)
                detect_idx = len(self.model.model) - 1
                detect_prefix = f"model.{detect_idx}"
                # Get the pretrained state dict from the checkpoint
                pretrained_state = self.ckpt["model"].state_dict()
                # Filter out any keys that belong to the detection head
                filtered_state = {k: v for k, v in pretrained_state.items() if not k.startswith(detect_prefix)}
                # Get the current model state and update it with the filtered pretrained weights
