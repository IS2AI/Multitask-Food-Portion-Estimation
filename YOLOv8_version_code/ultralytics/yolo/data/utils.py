...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                if keypoint:
                    assert lb.shape[1] == (8 + nkpt * ndim), f'labels require {(8 + nkpt * ndim)} columns each'
                ...
                ne = 1  # label empty
                lb = np.zeros((0, (1 + nkpt * ndim)), dtype=np.float32) if keypoint else np.zeros(
                    (0, 6), dtype=np.float32)
        else:
            nm = 1  # label missing
            lb = np.zeros((0, (1 + nkpt * ndim)), dtype=np.float32) if keypoint else np.zeros((0, 6), dtype=np.float32)
        if keypoint:
            keypoints = lb[:, 5:15].reshape(-1, nkpt, ndim)
            if ndim == 2:
