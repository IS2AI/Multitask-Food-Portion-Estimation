...
class RandomPerspective:
    ...
    def apply_bboxes(self, bboxes, M):
        ...
        x = xy[:, [0, 2, 4, 6]]
        y = xy[:, [1, 3, 5, 7]]
        result=np.concatenate((x.min(1), y.min(1), x.max(1), y.max(1)), dtype=bboxes.dtype).reshape(4, n).T
        b=bboxes[:,4:]
        add_out=np.concatenate((result,  bboxes[:,4:]), axis=1)
        return add_out
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels['bboxes'] = torch.from_numpy(instances.bboxes) if nl else torch.zeros((nl, 5))
