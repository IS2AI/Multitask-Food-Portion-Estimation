...
class DetectionTrainer(BaseTrainer):
    ...
    def get_validator(self):
        ...
        self.loss_names = 'box_loss', 'cls_loss', 'dfl_loss', 'weight_loss'
    ...
    def plot_training_labels(self):
        ...
        for lb in self.train_loader.dataset.labels:
            if lb['bboxes'].shape[1] == 5:
                valid_boxes.append(lb['bboxes'])
                valid_classes.append(lb['cls'])
            else:
                print(f"Skipping a label due to mismatched shape: {lb['bboxes'].shape}")

        boxes = np.concatenate(valid_boxes, axis=0)
        cls = np.concatenate(valid_classes, axis=0)
